...
export default {
  '*.{js,ts,jsx,tsx,cjs,mjs,json,html,xml,svg,css,scss,sass,md}':
    'cspell --no-progress --no-must-find-files',
  '*.{js,ts,jsx,tsx,cjs,mjs}': 'eslint --max-warnings 0 --fix',
  '*': 'prettier --write --ignore-unknown --log-level warn',
  'src/**/*.{js,ts,jsx,tsx}': (paths) => {
    const relativePaths = paths.map((filePath) =>
      path.relative(dirname, filePath).split(path.sep).join(path.posix.sep),
    );
